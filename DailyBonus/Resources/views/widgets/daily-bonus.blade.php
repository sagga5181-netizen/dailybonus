{{-- Resources/views/widgets/daily-bonus.blade.php --}}
    ...
    <div class="widget-body">
        @if(!$userBonus['isLoggedIn'])
            <div class="daily-bonus-guest">
                <p class="text-center text-muted">{{ __('dailybonus.please_login') }}</p>
                ...
                <div class="row text-center">
                    <div class="col-6">
                        <div class="stat-item">
                            ...
                            <span class="stat-label">{{ __('dailybonus.days_claimed') }}</span>
                        </div>
                    </div>
                    <div class="col-6">
                        <div class="stat-item">
                            <span class="stat-value">{{ $userBonus['totalClaimed'] ?? 0 }}</span>
                            <span class="stat-label">{{ __('dailybonus.total_received') }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
.widget-daily-bonus .widget-title {
    display: flex;
    align-items: center;
    gap: 10px;
    font-size: 18px;
...
.widget-daily-bonus .bonus-stats {
    background: rgba(255, 255, 255, 0.05);
    border-radius: 12px;
    padding: 15px;
}

.widget-daily-bonus .stat-item {
    display: flex;
    flex-direction: column;
    align-items: center;
}

.widget-daily-bonus .stat-value {
    font-size: 24px;
    font-weight: 700;
    color: #ffd700;
}

.widget-daily-bonus .stat-label {
    font-size: 12px;
    color: rgba(255, 255, 255, 0.6);
}

.widget-daily-bonus .days-grid {
    display: grid;
    grid-template-columns: repeat(7, 1fr);
    gap: 8px;
}

.widget-daily-bonus .bonus-day {
    aspect-ratio: 1;
    display: flex;
    flex-direction: column;
    align-items: center;
...
.widget-daily-bonus .bonus-day.claimed {
    background: linear-gradient(135deg, #28a745 0%, #20c997 100%);
}

.widget-daily-bonus .bonus-day.current {
    background: linear-gradient(135deg, #ffc107 0%, #ff9800 100%);
    animation: pulse 2s infinite;
}

.widget-daily-bonus .bonus-day.locked {
    opacity: 0.5;
}

.widget-daily-bonus .day-number {
    font-size: 12px;
    font-weight: 600;
    color: rgba(255, 255, 255, 0.8);
}

.widget-daily-bonus .bonus-day.claimed .day-number,
.widget-daily-bonus .bonus-day.current .day-number {
    color: #fff;
}

.widget-daily-bonus .day-amount {
    font-size: 10px;
    font-weight: 700;
    color: #ffd700;
}

.widget-daily-bonus .bonus-day.claimed .day-amount,
.widget-daily-bonus .bonus-day.current .day-amount {
    color: #fff;
}

.widget-daily-bonus .day-status {
    position: absolute;
    top: -4px;
    right: -4px;
    width: 16px;
...
.widget-daily-bonus .current-badge {
    position: absolute;
    bottom: -8px;
    left: 50%;
...
.widget-daily-bonus .countdown-timer {
    font-size: 28px;
    font-weight: 700;
    color: #ffd700;
...
function claimDailyBonus() {
    ...
    .catch(error => {
        showNotification('{{ __("dailybonus.errors.error") }}', 'error');
    });
}
